...
Case comes in
    ...
    │     └──→ delegate to `ponytail` for how minimal it should be. If the
    │           stack matches `biney-brain-stack`'s stack-skill table
    │           (Spring Boot, PostgreSQL, Angular, Power BI), pair it with
    │           that skill too for what's idiomatically correct there,
    │           ponytail decides how much, the stack skill decides what's
    │           right, never one instead of the other.
    │
    ...
    │
    ├── Verifying a UI/frontend change actually works (not writing/designing it)?
    │     └──→ `playwright-cli` by default: scriptable, no site permissions,
    │           produces a reusable artifact (test file, video, trace) instead
    │           of a one-off check. Switch to `claude-in-chrome` only when the
    ...
    │   score (e.g. a Turnitin-flagged thesis chapter)?
    │     └──→ mandatory: check specifically for the `humanizer` skill and
    │           run the document through it before handing it back, even if
    │           nothing about it seemed off. Don't substitute a manual rewrite
    │           for it. If `humanizer` isn't installed, say so and name the
    │           exact install path (`find-skills`, or the skill by name)
    │           instead of touching the document without it.
    │
...
- `humanizer`: strips AI writing patterns out of a document, generated by you or handed to you already written, mandatory per Step 1.
